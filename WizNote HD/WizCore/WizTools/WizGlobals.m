...
+ (NSURL*) wizServerUrl
{
    @synchronized(self)
    {
        ...
        if (serverUrl == nil) {
            ...
            if (data != nil) {
                NSString* urlStr = [NSString stringWithCString:(const char *)[data bytes] length:data.length];
                serverUrl = [NSURL URLWithString:urlStr];
            }
            NSDate* date2 = [NSDate date];
            NSLog(@"get url time %f",[date2 timeIntervalSinceDate:date1]);
        }
        if (serverUrl == nil) {
